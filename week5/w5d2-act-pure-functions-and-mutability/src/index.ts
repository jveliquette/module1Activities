...
function incrementCounter(): number {
  let counter: number = 0; // I moved this line of code from above the function to inside the function
  return counter++;
}
...
function purePercentNull(users: Users): NullStats {
  // let countNulls: number = 0;
  // let percent: number = 0;
  const nullIpAddressArray: boolean[] = users.map((user) => user.ip_address === null)
  const countNulls: number = nullIpAddressArray.filter((addressNull) => addressNull).length // same as = for (const addressNull of nullIpAddressArray)
  const percent: number = (countNulls / users.length) * 100;
  return { nullIpAddresses: countNulls, percent: `${percent}%` }
}
